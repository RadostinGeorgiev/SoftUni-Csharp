namespace _01._Trains_Part_Three
{
    using System;
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (nodes != 0 && tubes != 0)
            {
                maxFlow = ReconstructPath(source, destination);
            }
        ...
        {
            for (int node = 0; node < graph.EdgesCount; node++)
            {
                int[] tubeData = Console.ReadLine()
                    .Split()
                    .Select(int.Parse)
                    .ToArray();

                graph.Connect(tubeData[0], tubeData[1], tubeData[2]);
            }
            return graph;
        }
