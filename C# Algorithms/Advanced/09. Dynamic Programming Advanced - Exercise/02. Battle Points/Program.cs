namespace _02._Battle_Points
{
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            List<int> selectedItems = GetTakenItems(matrix);

            Console.WriteLine($"{selectedItems.Sum()}");
            //Console.WriteLine($"{string.Join(", ", selectedItems)}");
        }

        private static List<int> GetTakenItems(int[,] matrix)
        {
        ...
        {
            int[,] matrix = new int[energy.Length + 1, initialPoints + 1];
            ...
            for (int index = 1; index < matrix.GetLength(0); index++)
            {
                for (int capacity = 1; capacity < matrix.GetLength(1); capacity++)
                {
                    ...
                    {
                        int including = points[index - 1] + matrix[index - 1, capacity - energy[index - 1]];
